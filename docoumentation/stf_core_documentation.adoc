...
By itself, STF is merely a 'shell' format. Actual content is stored in objects defined by their `type`. Support for importing, exporting & validationg a resource type is implemented by a module.

The minimum required set of modules is specified in link:./core_modules.adoc[Core Modules].

STF implementations must provide an easy to use module plugin system, and if in any way possible, the ability to hot-load modules.

Objects of any type get referenced by a unique ID as a string. This will be refered to as `ID`. Usually it should be a UUID v4.
...
Resources may be composed of other resources. The top most resource is exclusively responsible for listing all contained references to other resources and buffers.

Resources can be `Data`, `Node`, `Component` and `Modification` kinds.
...
* no_prefab_instances
This asset doesn't contain any instantiations of prefabs. Armatures may be instanced once throughout the entire asset.

* compatibility_game_engine
This profile applies the conditions of: `mesh_max_8_weights`, `node_names_not_empty`, `precision_32_bit_only`.

* compatibility_game_engine_wide
This profile applies the conditions of: `mesh_max_4_weights`, `node_names_not_empty`, `precision_32_bit_only`.

* compatibility
This profile applies the conditions of: `mesh_max_8_weights`, `node_names_unique_within_prefab`, `mesh_max_8_uv_channels`, `precision_32_bit_only`.

* compatibility_wide
This profile applies the conditions of: `mesh_max_4_weights`, `node_names_unique_within_prefab`, `mesh_max_8_uv_channels`, `precision_32_bit_only`.
