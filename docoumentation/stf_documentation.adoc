...
It specifies the index of the buffer in within the STF binary with the `index` property.

In a `stf.json` file, `stf.buffer.file` is the only supported buffer type. It references a `.stfbuffer` file with a relative path with its `path` property.
